...
impl Default for Configuration {
    fn default() -> Self {
        Configuration {
            backdoor_size: 10,
            num_iters: 10000,
            stagnation_limit: Some(1000),
            derive_ternary: false,
            max_product: 10000,
            use_sorted_filtering: false,
            num_conflicts: 1000,
            budget_filter: 10000,
            factor_budget_filter: 1.1,
            budget_solve: 100000,
            factor_budget_solve: 1.1,
        }
    }
}
...
fn main() -> color_eyre::Result<()> {
    ...
    let int_118 = Configuration {
        backdoor_size: 10,
        num_iters: 10000,
        max_product: 10000,
        use_sorted_filtering: true,
        budget_filter: 10000,
        factor_budget_filter: 1.1,
        budget_solve: 100000,
        factor_budget_solve: 1.2,
        ..Default::default()
    };
    let int_121 = Configuration {
        backdoor_size: 10,
        num_iters: 10000,
        max_product: 10000,
        use_sorted_filtering: true,
        ...
        factor_budget_filter: 1.1,
        budget_solve: 100000,
        factor_budget_solve: 1.3,
        ..Default::default()
    };
    let int_125 = Configuration {
        backdoor_size: 10,
        num_iters: 10000,
        max_product: 10000,
        use_sorted_filtering: true,
        ...
        factor_budget_filter: 1.1,
        budget_solve: 100000,
        factor_budget_solve: 1.4,
        ..Default::default()
    };
    let int_126 = Configuration {
        backdoor_size: 10,
        num_iters: 10000,
        max_product: 10000,
        use_sorted_filtering: true,
        ...
        factor_budget_filter: 1.1,
        budget_solve: 100000,
        factor_budget_solve: 1.5,
        ..Default::default()
    };
    let config: Configuration = match args.folio_config.as_str() {
        "int_118" => int_118,
        "int_121" => int_121,
        "int_125" => int_125,
        "int_126" => int_126,
        _ => {
            bail!("Unknown configuration: {}", args.folio_config);
        }
    };
    ...
    if !_unsat && final_model.is_none() {
        ...
        if num_vars < args.folio_low as i64 || num_vars > args.folio_high as i64 {
    ...
    loop {
        ...
        if let Some(result) = searcher.run(
            ...
            if !args.no_derive {
                ...
                let derived_clauses = derive_clauses(&hard, config.derive_ternary);
            ...
            if !args.no_derive {
                ...
                let time_derive = Instant::now();
                let derived_clauses = derive_clauses(&cubes_product, config.derive_ternary);
                let time_derive = time_derive.elapsed();
                info!(
            ...
            if cubes_product.len() > config.max_product {
                info!(
        ...
        if config.use_sorted_filtering {
                ...
                if best_cube_score > 0.0 {
                    ...
                    match &searcher.solver {
                        SatSolver::Cadical(solver) => {
                            ...
                            solver.limit("conflicts", (config.num_conflicts as u64) as i32);
            ...
            cubes_product.retain(|cube| {
                ...
                match &searcher.solver {
                    SatSolver::Cadical(solver) => {
                        ...
                        solver.limit("conflicts", config.num_conflicts as i32);
                        let time_solve = Instant::now();
                        let res = solver.solve().unwrap();
                        let time_solve = time_solve.elapsed();
        ...
        budget_filter = (budget_filter as f64 * config.factor_budget_filter) as u64;
        ...
        budget_solve = (budget_solve as f64 * config.factor_budget_solve) as u64;
